...
def scan_all_hosts(subnet):
    ...
    if ':' in subnet:
        nm.scan(hosts=subnet, arguments='-n -Pn --open -O -6 --min-rate 5000 --max-retries 1 --host-timeout 5s -p 22,3389,5985,5986')
    else:
        nm.scan(hosts=subnet, arguments='-n -Pn --open -O --min-rate 5000 --max-retries 1 --host-timeout 5s -p 22,3389,5985,5986')
...
def windows_port_scan_only(host):
    ...
    try:
        ...
        # Use -6 flag for IPv6 scanning
        if ':' in host:
            ps.scan(hosts=host, arguments='-n --max-rto 1000 -Pn -p 21,22,23,53,67,80,123,389,443,445,1500,3389,5985,5986 -6')
        else:
            ps.scan(hosts=host, arguments='-n --max-rto 1000 -Pn -p 21,22,23,53,67,80,123,389,443,445,1500,3389,5985,5986')
        port_dict = {
            21: "FTP",
            22: "SSH",
            23: "Telnet",
...
def linux_port_scan_only(host):
    ...
    try:
        ...
        # Use -6 flag for IPv6 scanning
        if ':' in host:
            ps.scan(hosts=host, arguments='-n --max-rto 1000 -Pn -p 1-65535 -6')
        else:
            ps.scan(hosts=host, arguments='-n --max-rto 1000 -Pn -p 1-65535')
        port_dict = {
            21: "FTP",
            22: "SSH",
            23: "Telnet",
